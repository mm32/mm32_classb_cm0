...
static void MCO_Config(void)
{
    ...
    RCC->CFGR2 &= ~(0xF << 20);
    RCC->CFGR2 |= RCC_CFGR2_MCO_PRE_128; // MCO = HSE/128
    RCC_MCOConfig(RCC_CFGR_MCO_HSE);
#elif defined (HSI_INCAP)
    RCC_HSICmd(ENABLE);
    while (RCC_GetFlagStatus(RCC_FLAG_HSIRDY) == RESET);

    RCC->CFGR2 &= ~(0xF << 20);
    RCC->CFGR2 |= RCC_CFGR2_MCO_PRE_128; // MCO = HSI/128
...
void Tim14_UPCount_test(u16 Prescaler, u16 Period)
{
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1ENR_TIM14 | RCC_APB1ENR_SYSCFG, ENABLE);
...
void WWDG_IRQHandler(void)
{
    ...
    IWDG_ReloadCounter();
    EXTI_ClearITPendingBit(EXTI_Line17); //Clear EXTI line-24 IWDG interrupt.
}
...
void TIM14_IWDG_Xcross_Measure_Init(void)
{
    ...
    Tim14_UPCount_test(48-1, 0xFFFF);
...
#ifdef USE_TIM3_CLOCK_CHECK
    MCO_Config();          // MCO output - PA7
    TIM3_PWM_Input_Init(); // TIM3_CH2 input- PA2
#else
