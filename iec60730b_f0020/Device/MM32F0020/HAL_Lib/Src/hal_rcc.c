...
///      RCC_AHBENR_GPIOA, RCC_AHBENR_GPIOB,RCC_AHBENR_CRC
...
void RCC_ADC_ClockCmd(ADC_TypeDef* peripheral, FunctionalState state)
{
    switch (*(vu32*)&peripheral) {
        ...
        case ADC1_BASE:
            (state) ? (RCC->APB1ENR |= RCC_APB1ENR_ADC1) : (RCC->APB1ENR &= ~RCC_APB1ENR_ADC1);
            break;
        default:
            break;
    }
...
void RCC_UART_ClockCmd(UART_TypeDef* peripheral, FunctionalState state)
{
    ...
    if(UART1 == peripheral) {
        (state) ? (RCC->APB1ENR |= RCC_APB1ENR_UART1) : (RCC->APB1ENR &= ~RCC_APB1ENR_UART1);
    }
}
/// @}
