...
void TIM2_Init(void)
{
    ...
    TIM2->OR = (3<<6); // HSE/128 as TIM2_CH4 capture input
#endif

    RCC_APB1PeriphClockCmd(RCC_APB1ENR_PWR, ENABLE); //Enable power register clocked
...
void Tim14_UPCount_test(u16 Prescaler, u16 Period)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2ENR_TIM14 | RCC_APB2ENR_SYSCFG, ENABLE);
...
void WWDG_IWDG_IRQHandler(void)
{
    ...
    if (LSIPeriodFlag == 0u)
    {
        ...
        if (tmpCC4_now > tmpCC4_last)
        {
            ...
            //printf("1.Period:%d", PeriodValue);
        }
        else
        {
            PeriodValue = (0xFFFF - tmpCC4_last + tmpCC4_now)& 0xFFFFuL;
             //printf("2.Period:%d", PeriodValue);
        }
    ...
    IWDG_ClearIT();
    IWDG_ReloadCounter();
    EXTI_ClearITPendingBit(EXTI_Line24); //Clear EXTI line-24 IWDG interrupt.
}
...
void TIM14_IWDG_Xcross_Measure_Init(void)
{
    ...
    Tim14_UPCount_test(SYSTCLK_AT_STARTUP/1000000-1, 0xFFFF);
    ...
    NVIC_StructInit.NVIC_IRQChannel = WWDG_IWDG_IRQn;
...
#else
    /* USING TIM and IWDG to check clock. */
